...
function weixin_robot_admin_menu() {
	//add_menu_page($weixin_robot_name, $weixin_robot_name,	'manage_options',	'weixin-robot',	'weixin_robot_basic_page',	WEIXIN_ROBOT_PLUGIN_URL.'/static/weixin-16.ico');
	...
	//add_menu_page($weixin_robot_name, $weixin_robot_name,	'manage_options',	'weixin-robot',	'weixin_robot_basic_page',	WEIXIN_ROBOT_PLUGIN_URL.'/static/weixin.svg');
	
	//add_menu_page($weixin_robot_name, $weixin_robot_name,	'manage_options',	'weixin-robot',	'weixin_robot_basic_page', 'dashicons-format-chat');
...
<?php
	...
	if(strpos($plugin_page, 'weixin-robot') !== false){
?>
<style type="text/css">.widefat td {  vertical-align: middle;}</style>
<?php
	}
}
...
function weixin_robot_get_basic_option_labels(){
    ...
    $default_reply_section_fields = array(
		...
		'weixin_not_found'				=> array('title'=>'没有匹配时回复',	'type'=>'textarea', 'rows'=>5,	'description'=>'可以使用 [keyword] 代替相关的搜索关键字，留空则不回复！'),
...
function weixin_robot_datas_page() {
	...
	<div class="wrap">
		...
		$sql = "DESCRIBE " . $weixin_custom_replies_table . " 'match'";
		if($wpdb->query($sql) == 0){
