...
function weixin_robot_credit_reply(){
	...
	$profile_link = home_url('/?weixin_user_profile&'.$query_key.'='.$query_id);
...
function weixin_robot_daily_credit_checkin($weixin_openid){ // 过了 24 小时才能签到
	...
	if($last_checkin_time === false){
		global $wpdb;
		$weixin_credits_table = weixin_robot_get_credits_table();
		$last_checkin_time = $wpdb->get_var($wpdb->prepare("SELECT `time` FROM {$weixin_credits_table} WHERE `type`=%s AND weixin_openid=%s ORDER BY id DESC LIMIT 1",$type,$weixin_openid));
...
function weixin_robot_daily_credit_checkin_2($weixin_openid){ //过了 0 点就能签到
	...
	if($has_checkin === false){
		...
		$last_checkin_time = $wpdb->get_var($wpdb->prepare("SELECT `time` FROM {$weixin_credits_table} WHERE `type`=%s AND weixin_openid=%s AND ( ( YEAR( time ) = %d AND MONTH( time ) = %d AND DAYOFMONTH( time ) = %d ) )  ORDER BY id DESC LIMIT 1", $type, $weixin_openid, date('Y',$current_time), date('m',$current_time), date('d',$current_time) ));
